...
const props = defineProps({
  ...
  fullSearchField: {
    type: [Array, String],
    default: () => ['name','code','modelCode','description']
  },
  columns: {
    type: Array,
    ...
    >
      <div class="tools-box">
        ...
        <div class="search-box">
          <span class="text">全表查询</span>
          <el-input v-model="searchText" class="search-input" placeholder="请输入关键字" clearable @clear="onSearchClear" @input="onSearch">
      ...
      >
        ...
        <vxe-table-column title="名称" field="name" sortable width="260" tree-node>
          <template #default="{ row }">
            <span class="role-name-box">
              <template v-if="row[childrenField] && row[childrenField].length">
        ...
        >
          <template v-if="col.field===STATUS_FIELD" #default="{ row }">
            <!-- 0,1,2            -->
            <span>{{ row[STATUS_FIELD] === 0? '未启用' : row[STATUS_FIELD]===1?'启用':'禁用' }}</span>
          </template>
        </vxe-table-column>
      </vxe-table>
