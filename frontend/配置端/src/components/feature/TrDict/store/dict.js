...
import { queryDictItemByCodes } from '@/views/dictionary-manage/api/index.js'
        ...
        httpRequest(queryTypes.join(',')).then(({ data }) => {
          if (data) {
            data.forEach((item) => {
              ...
              commit('SET_DICT', { type: item.code, data: item.dictionaryItems.map(item => {
                // 禁用的字典项
                item.disabled = item.enableFlag === 2

                return item
              }).filter(o => o.enableFlag !== 0) }) // 未启用
            })
          }
