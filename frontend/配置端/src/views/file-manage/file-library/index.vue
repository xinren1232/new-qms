...
function onAddOrEditClick(row) {
  router.push(`/file-manage/file-library/${row.bid || 'add'}`)
}
...
function fileStorageAddressGetters({ address }) {
  return store.getters.dict.FILE_STORAGE_ADDRESS_MAP?.[address]
}
...
const updateState = (params) => {
  ...
  return updateFileLibraryStatus(_params)
}

onMounted(() => {
  store.dispatch('dict/queryDict', 'FILE_STORAGE_ADDRESS')
})
</script>
...
<template>
  <div class="">
    ...
    <tr-table
      ...
      immediate
    >
      <vxe-table-column title="文件库名称" field="name" align="center" />
      <!--      <vxe-table-column title="URL规则" field="urlRule" align="center" />-->
      <vxe-table-column title="文件库URL" field="url" align="center" />
      <vxe-table-column title="归属地" field="address" align="center">
        <template #default="{ row }">
          {{ fileStorageAddressGetters(row) }}
        </template>
      </vxe-table-column>
      <vxe-table-column title="文件库描述" field="description" align="center" show-overflow />
      <vxe-table-column title="是否默认库" field="defaultFlag" align="center" width="100">
        <template #default="{ row }">
          {{ row.defaultFlag ? '是' : '否' }}
        </template>
      </vxe-table-column>
      <vxe-table-column title="状态" field="enableFlag" align="center" width="200">
        <template #default="{ row }">
          <tr-switch
