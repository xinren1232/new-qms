...
const updateState = (params) => {
  ...
  return updateCopyRuleStatus(_params)
}

// 点击新增或编辑按钮
function onAddOrEditClick(row) {
  router.push(`/file-manage/copy-rule/${row.bid || 'add'}`)
}
...
async function onDeleteData(row) {
  ...
  deleteCopyRuleFileLibrary(row.bid)
    .then(({ success }) => {
      if (success) {
        onRefreshTableData()
...
<template>
  <div class="">
      ...
      <el-popover @hide="onRefreshTableData()">
        ...
        <div class="p-2">
          <el-form label-position="right" label-width="100px">
            ...
            <el-form-item label="复制事件">
              <tr-select v-model="filterForm.copyEvent" :data="copyEventList" />
            </el-form-item>
            <el-form-item label="复制模式">
              <tr-select v-model="filterForm.copyMode" :data="copyModeList" />
            </el-form-item>
            <el-form-item label="复制类型">
              <tr-select v-model="filterForm.copyType" :data="copyTypeList" />
            </el-form-item>
          </el-form>
    ...
    <tr-table
      ...
      immediate
    >
      <vxe-table-column title="复制规则名称" field="name" align="center" />
      <vxe-table-column title="复制事件" field="copyEvent" align="center">
        <template #default="{row}">
          {{ copyEventMap[row.copyEvent] }}
        </template>
      </vxe-table-column>
      <vxe-table-column title="复制模式" field="copyMode" align="center">
        <template #default="{row}">
          {{ copyModeMap[row.copyMode] }}
        </template>
      </vxe-table-column>
      <vxe-table-column title="复制类型" field="copyType" align="center">
        <template #default="{row}">
          {{ copyTypeMap[row.copyType] }}
        </template>
      </vxe-table-column>
      <vxe-table-column title="启用" field="name" align="center" width="200">
        <template #default="{ row }">
          <tr-switch
