...
import { getFileTypeList, deleteFileType, updateFileTypeStatus } from '@/api/file-manage.js'
...
const updateState = (params) => {
  ...
  return updateFileTypeStatus(_params)
}

// 点击新增或编辑按钮
function onAddOrEditClick(row) {
  router.push(`/file-manage/file-type/${row.bid || 'add'}`)
}
...
const renderStorageRule = (row) => {
  if (row.storageRule === 'IP') {
    return 'IP归属地'
  } else if (row.storageRule === 'Base') {
    return 'Base归属地'
  } else {
    return ''
  }
...
<template>
  <div class="">
    ...
    <tr-table
      ...
      immediate
    >
      <vxe-table-column title="文件类型名称" field="name" align="center" />
      <vxe-table-column title="后缀名" field="suffixName" align="center" />
      <vxe-table-column title="描述" field="description" align="center" show-overflow />
      <vxe-table-column title="优先级" field="priority" align="center" />
      <vxe-table-column title="存储规则" field="storageRule" align="center">
        <template #default="{row}">
          {{ renderStorageRule(row) }}
        </template>
      </vxe-table-column>
