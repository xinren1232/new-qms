<template>
  ...
  >
    ...
    <div slot="reference" class="btn-box" @click="visible=true"><tr-svg-icon icon-class="t-filter" /><span class="text"><span v-if="filterNumber>0" class="filter-rule-number">{{ filterNumber }}</span>筛选</span></div>
  </el-popover>
</template>
...
const formItems = ref(
  [
    { label: '关系名称', prop: 'name', labelWidth: '80px', span: 24, attrs: { placeholder: '请输入', clearable: true } },
    { label: 'TAB名称', prop: 'tabName', labelWidth: '80px', span: 24, attrs: { placeholder: '请输入', clearable: true } },
    {
      label: '状态',
      prop: 'enableFlag',
      labelWidth: '80px',
      component: TrSelect,
      span: 24,
      attrs: { data: STATUS, placeholder: '请选择状态', clearable: true }
    }
  ]
)
