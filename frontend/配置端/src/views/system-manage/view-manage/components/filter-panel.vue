...
const formItems = ref(
  [
    { label: '视图名称', prop: 'name', labelWidth: '80px', span: 24, attrs: { placeholder: '请输入', clearable: true } },
    { label: '视图类型', prop: 'type',component: TrDict, labelWidth: '80px', span: 24, attrs: { type: 'VIEW_TYPE', placeholder: '请输入', clearable: true } },
    {
      label: '状态',
      prop: 'enableFlag',
      labelWidth: '80px',
...
// 筛选
const onFilter = () => {
  if (form.value?.enableFlag)form.value.enableFlag = Number(form.value.enableFlag)
  emit('filter-change', form.value)
  calcFilterNumber()
  visible.value = false
}
