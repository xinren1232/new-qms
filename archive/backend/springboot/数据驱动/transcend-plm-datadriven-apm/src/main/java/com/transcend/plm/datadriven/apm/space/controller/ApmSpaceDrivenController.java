 ...
 **/
@RestController
@Api(value = "APM Controller", tags = "空间-控制器")
@RequestMapping(value ="/apm/space")
public class ApmSpaceDrivenController {
     ...
     * 获取空间列表+应用
     * @return 响应
     * @deprecated 因为使用listApmSpaceAndApp 这个方法替代
     */
    @ApiOperation("获取空间列表+应用")
    ...
    @ApiOperation("获取空间列表+应用")
    @GetMapping("/getDefaultApmSpace")
    public TranscendApiResponse<ApmSpaceVo> getDefaultApmSpace(){
        return TranscendApiResponse.success(apmSpaceApplicationService.getDefaultApmSpace());
    }

    @ApiOperation("获取空间列表+应用")
    @PostMapping("/listApmSpacePage")
    public TranscendApiResponse<PagedResult<ApmSpaceVo>> listApmSpacePage(@RequestBody BaseRequest<ApmSpaceQo> pageRequest){
        return TranscendApiResponse.success(apmSpaceApplicationService.listApmSpacePage(pageRequest));
    }

    @ApiOperation("获取空间列表+应用")
    @PostMapping("/listAppByBids")
    public TranscendApiResponse<List<ApmSpaceAppVo>> listAppByBids(@RequestBody List<String> bids){
        return TranscendApiResponse.success(apmSpaceApplicationService.listAppByBids(bids));
    }

    @ApiOperation("获取空间列表+应用")
    @PostMapping("/queryApmSpaceAppInstance")
    public TranscendApiResponse<List<ApmSpaceAppQueryVo>> queryApmSpaceAppInstance(@RequestBody ApmSpaceAppQueryDto apmSpaceAppQueryDto){
        return TranscendApiResponse.success(apmSpaceApplicationService.queryApmSpaceAppInstance(apmSpaceAppQueryDto));
    }
    ...
    @ApiOperation("空间配置 - 查询角色&权限树")
    @PostMapping("/listRoleAndPermissionTree")
    public TranscendApiResponse<List<ApmRoleVO>> listRoleAndPermissionTree(@RequestBody ApmSpaceRoleQO apmSpaceRoleQo){
        return TranscendApiResponse.success(apmSpaceApplicationService.listRoleAndPermissionTree(apmSpaceRoleQo));
    }
    ...
    @ApiOperation("空间配置 - 操作应用（删除、启用、禁用、修改、隐藏）")
    ...
    @ApiOperation("获取应用图标")
    @PostMapping("/getIconByBids")
    public TranscendApiResponse<List<ApmSpaceAppVo>> gerIconByBids(@RequestBody List<String> bids){
        List<ApmSpaceAppVo> apmSpaceAppVos = apmSpaceApplicationService.listAppByBids(bids);
        if (CollectionUtils.isNotEmpty(apmSpaceAppVos)) {
            apmSpaceAppVos = apmSpaceAppVos.stream().map(apmSpaceAppVo -> {
                ApmSpaceAppVo apmSpaceAppVo1 = new ApmSpaceAppVo();
                apmSpaceAppVo1.setIconUrl(apmSpaceAppVo.getIconUrl());
                apmSpaceAppVo1.setBid(apmSpaceAppVo.getBid());
                apmSpaceAppVo1.setName(apmSpaceAppVo.getName());
                return apmSpaceAppVo1;
            }).collect(Collectors.toList());
        }
        return TranscendApiResponse.success(apmSpaceAppVos);
    }
}
