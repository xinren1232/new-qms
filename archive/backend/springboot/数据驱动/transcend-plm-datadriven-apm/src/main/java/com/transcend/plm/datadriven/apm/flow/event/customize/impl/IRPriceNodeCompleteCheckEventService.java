 ...
 * @author yuanhu.huang
 * @date 2024/08/13
 * @description IR价值节点校验
 */
@Service("iRPriceNodeCompleteCheckEventService")
public class IRPriceNodeCompleteCheckEventService implements IFlowCompleteCheckEvent {
    ...
    public void check(ApmSpaceApp app, ApmFlowInstanceNode currentNode, MObject mObject) {
       //校验 RAT评审的需求价值、产品方案、技术可行性评估，并且单选，三种类型可重复创建，点击提交时必须校验，三种评审类型存在，并且三种评审结果为：GO或者GO with risk
        ...
        //对应的IR评估结论数据进行校验
        //当前优先级
        String requirementlevel = mObject.get("requirementlevel")+"";
        ...
        for (MObject tarMobject : mObjects) {
            ...
            if(FlowCheckEnum.RMTREVIEW.getCode().equals(reviewType) && veviewResults.contains(reviewResults)){
                if(FlowCheckConstant.DEMANDVALUE.equals(rmtreviewcontent)){
                    aAndSPass = true;
                }
            }
