...
@Service
@AllArgsConstructor
public class AlmDataDeliverResponsibilityFieldConvertImpl implements AlmDataDeliverSpecialConverter {
    private final String FIELD = "responsibilityField";
    ...
    public void convert(TranscendObjectWrapper data) {
        ...
        if (value instanceof String) {
            ...
            MObject object = objectModelCrudI.getByBid(TranscendModel.DUTY_FIELD.getCode(), strValue);
        ...
        if (value instanceof Collection) {
            ...
            List<MObject> list = objectModelCrudI.list(TranscendModel.DUTY_FIELD.getCode(),
                    QueryWrapper.buildSqlQo(wrapper));
            Map<String, Object> dataMap = list.stream().map(TranscendObjectWrapper::new)
                    .collect(Collectors.toMap(TranscendBaseWrapper::getBid, t -> t.get(outputField)));
            List<Object> resultList = bidList.stream().map(bid -> dataMap.getOrDefault(bid, bid)).collect(Collectors.toList());
            data.put(FIELD, resultList);
        }
    }
